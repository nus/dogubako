...
type ImageTool struct {
	...
	beforeZoom    previewZoomBar
	afterLabel    basicwidget.Text
	afterImage    destPreview
	afterZoom     previewZoomBar
	...
	beforeHead   []guigui.LinearLayoutItem
	afterItems   []guigui.LinearLayoutItem
	afterHead    []guigui.LinearLayoutItem
	editItems    []guigui.LinearLayoutItem
	outputItems  []guigui.LinearLayoutItem
	layoutItems  []guigui.LinearLayoutItem
	beforeHeader guigui.LinearLayout
	afterHeader  guigui.LinearLayout
...
func (t *ImageTool) Build(context *guigui.Context, adder *guigui.ChildAdder) error {
	...
	if out, err := model.Processed(); err == nil && out != nil {
		...
		t.afterZoom.Configure(context, &t.afterImage, lang, false)
...
func (t *ImageTool) Layout(context *guigui.Context, widgetBounds *guigui.WidgetBounds, layouter *guigui.ChildLayouter) {
	...
	t.beforeHead = slices.Delete(t.beforeHead, 0, len(t.beforeHead))
	t.beforeHead = append(t.beforeHead,
		guigui.LinearLayoutItem{Widget: &t.beforeLabel, Size: guigui.FlexibleSize(1)},
		guigui.LinearLayoutItem{Widget: &t.beforeZoom},
	)
	t.beforeHeader = guigui.LinearLayout{Direction: guigui.LayoutDirectionHorizontal, Items: t.beforeHead, Gap: u / 4}

	t.afterHead = slices.Delete(t.afterHead, 0, len(t.afterHead))
	t.afterHead = append(t.afterHead,
		guigui.LinearLayoutItem{Widget: &t.afterLabel, Size: guigui.FlexibleSize(1)},
		guigui.LinearLayoutItem{Widget: &t.afterZoom},
	)
	t.afterHeader = guigui.LinearLayout{Direction: guigui.LayoutDirectionHorizontal, Items: t.afterHead, Gap: u / 4}

	t.beforeItems = slices.Delete(t.beforeItems, 0, len(t.beforeItems))
	t.beforeItems = append(t.beforeItems,
		guigui.LinearLayoutItem{Size: guigui.FixedSize(u), Layout: &t.beforeHeader},
	...
	t.afterItems = append(t.afterItems,
		guigui.LinearLayoutItem{Size: guigui.FixedSize(u), Layout: &t.afterHeader},
