...
- 加工前 / 加工後のプレビュー（表示は拡大縮小できます）

プレビューの表示倍率は、パネル見出しの右にある **−** / **＋** / **全体** のボタンと、プレビュー上のマウスホイールで変えられます。倍率はボタンの左に % で出ます。**全体** で画像全体が収まる表示に戻ります。加工前と加工後は別々に拡大縮小できます。

画像が表示領域より大きくなったら、ドラッグで見る場所を動かせます。加工前のプレビューは左ドラッグが切り取りなので、移動は中ボタンのドラッグか Shift を押しながらの左ドラッグです。加工後のプレビューは左ドラッグでそのまま動きます。400% を超えると補間せずに描くので、ドットの境目がぼやけません。
...
- プレビューの拡大縮小（**−** / **＋** / **全体** のボタン、マウスホイール、拡大中は左ドラッグで移動）
